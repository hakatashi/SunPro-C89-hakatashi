...
復元には様々な手法が考えられますが、今回は以下の手順で復元を試みました。
 ...
 1. 啄木の既存の歌集からマルコフ連鎖で「大跨に緣側を歩けば」で始まる適当な文章を生成する
 2. 生成した文章のうち、文節に区切った時に五・七・七の条件を満たしている物を抽出し、これらを「最後の一首」の候補歌とする
 3. これとは別に啄木の短歌から一首ずつ特徴量を抽出してナイーブベイズによる分類器を作る
 4. 「最後の一首」の候補歌を分類器にかけ、最もスコアが高いものを復元された「最後の一首」とする
